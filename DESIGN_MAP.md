...
**Render:** grid de puntos en world coords con spacing 50 unidades + marker más grande en world (0,0). **C23/C24 (receta final con fix de stacking context):** `mix-blend-mode: exclusion` aplicado al canvas + dots `rgba(255,255,255,0.4)` + center marker `rgba(255,255,255,0.6)`. Replica la receta "Exclusion + 20% relleno" de Photoshop: gris suave sobre papel claro, gris-blancuzco sobre dark mode, contraste sutil sobre colores intermedios — sin glare en ningún caso. Switch light/dark mode eliminado del componente (un solo color base, blend mode lo adapta). Reactiva a `drawingZoom`, `drawingPan`, container resize (vía ResizeObserver) — `isDarkMode` eliminado del dependency array.

**C24 — fix crítico de stacking context:** el wrapper `<div>` del overlay tenía `zIndex: 1` originalmente, lo que creaba un nuevo stacking context aislado. El `mix-blend-mode` del canvas hijo solo blendea con backdrop dentro del padre stacking context — al estar el wrapper aislado, el canvas no podía ver StrataCanvas (sibling con z-0) como backdrop, y los dots se pintaban como blancos translúcidos sin blend real. **Solución:** eliminar `zIndex: 1` del wrapper. El wrapper queda sin stacking context propio (`position: absolute` + `z-auto`), y el canvas del overlay encuentra a StrataCanvas como backdrop dentro del stacking context root. El orden visual (overlay encima de StrataCanvas) se preserva por orden DOM: `<CompositionGuideOverlay />` viene después de `<StrataCanvas />` en App.tsx → pinta encima naturalmente. ControlsV2 / modales tienen sus propios `position: fixed` con z-index altos → quedan por encima del overlay como debe ser.

(Evolución: C21 dual rgba con switch dark/light → C22 white sólido + difference [demasiado intrusivo en dark, invisible sobre papel] → C23 exclusion + alpha reducido [teóricamente correcto pero blend mode no se aplicaba] → C24 fix de stacking context que finalmente activa el blend).  
...
**WelcomeModalV2 — jerarquía dos niveles + tipografía expandida unificada (C21/C22):** `ResourceLink` acepta prop opcional `mutedColor?: string`. Sin prop → **NIVEL 1** (color `T.purple`, underline solo en hover). Con prop → **NIVEL 2** (color muted, underline permanente). **C22 — restructura final del bloque inferior:** Zonas 3/4/5/6 colapsadas en **2 contenedores semánticos**: NIVEL 1 (fontWeight 500, sin underline) con 2 líneas — Línea 1: `Watch tutorial · by @dumaker` (separador `·` muted), Línea 2: `Support on Ko-fi 🤍`. NIVEL 2 (fontWeight 400, underline permanente, marginTop 14, color muted) con 3 líneas — `Inspired by Graintouch`, `Found a bug? Email me.` (button), `Keyboard shortcuts` (toggle con hasFinePointer guard + chevron-up/down + sección expandida desplegable). Sin gap interno entre items NIVEL 2 (lineHeight 1.5 da el espaciado). FontFamily unificada a `TYPE.numericValue.family` en ambos niveles. Sección expandida unificada a 10px en todos los elementos: headers FILE/EDIT/VIEW con fontWeight 400 + letterSpacing 0.06em + marginBottom 6, gap entre categorías 12, rows con gap 14 + fontSize 10 + lineHeight 20px, items label fontSize 10 explícito, items shortcut fontWeight 500.
