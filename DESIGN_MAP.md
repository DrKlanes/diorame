...
**Render:** grid de puntos en world coords con spacing 50 unidades + marker más grande en world (0,0). **C22:** color sólido `'white'` para ambos dots y center marker + `mix-blend-mode: difference` aplicado al canvas → los puntos se invierten automáticamente sobre cualquier color del backdrop (papel, dark, blanco brillante, azul claro). Switch light/dark mode eliminado (ya no necesario). Reactiva a `drawingZoom`, `drawingPan`, container resize (vía ResizeObserver) — `isDarkMode` eliminado del dependency array.  
