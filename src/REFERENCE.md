...
| `moveGizmoInteraction.ts` | ~185 | `hitTestGizmo`, `computeMoveTransform`, `isDragEngaged`: Move tool hit-testing + transform math (translate/rotate/scale uniform + scaleX/scaleY non-uniform for squash & stretch), plus the screen-space drag dead-zone (3px from pointerdown, hysteresis once crossed) that gates whether a gesture writes `currentTransform` at all. Pure module extracted from StrataCanvas |
