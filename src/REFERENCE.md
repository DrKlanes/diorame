...
| `gizmoCursor.ts` | ~80 | `cursorClassForGizmoMode`: maps a gizmo hit-test mode to the CSS cursor that announces the drag before the press. Rotation-aware — reads the real on-screen drag direction from the already-projected handles (`handle − center`) and rounds to one of the 8 native directional resize cursors; `cursor-rotate` (custom data-URI rule in `styles/globals.css`) for the rotation handle. Pure module |
