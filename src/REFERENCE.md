...
| `layers/LayersDiscoveryTooltip.tsx` | First-session layers-discovery tooltip UI (v3.17.32): reuses `usePopoverPosition` + `createPortal` directly (not the full `DiSelectorPopover`, which is listbox-shaped) with `placement="top"` hardcoded — `'auto'` resolved `'bottom'` against the viewport edge and overlapped the bottom-right `ColorPalette` pill, since the positioning hook has no notion of other fixed panels. Solid `T.purple` background + white text (v3.17.33) — a surface matching the panel theme read as "another panel," not a message; same brand token as `DiButton variant="brand"` / `DiModalActions` primary buttons. Close button forced `dark={true}` regardless of app theme, since it always sits on this solid-purple surface. Click-outside excludes the drawing canvas (v3.17.34, `target.closest('[data-drawing-canvas]')`) — resting a pen/finger on the canvas to draw is a `pointerdown` outside the tooltip too, and was dismissing it before the user could read it; `CompositionGuideOverlay`'s canvas sits on top but is `pointer-events:none` so it never becomes the event target |
...
| `useLayersDiscoveryTooltip.ts` | First-session-only contextual tooltip pointing at the layers panel (v3.17.32): fires once a user who drew a first real stroke (module-level flag, immune to `LayersPanel` remounts on `isUIHidden` toggle — see note in the file) crosses `TOOLTIP_TRIGGER_SHAPES`/`_SECONDS` with a single layer, no modal open, and no `.dior`/autosave load this session. Auto-fades after `TOOLTIP_VISIBLE_SECONDS` (v3.17.33) — drawing while it's visible no longer dismisses it (the original stroke-based auto-close killed it before the user could read it, since it opens mid-drawing); closes immediately if `totalLayers` increases while open (`layer_added` — the tooltip already did its job). Returns `{ isOpen, dismiss }` to `LayersPanel` |
...
## Appendix C: Changelog Highlights (1.7.3 → 3.14.1)

### 3.14.1 — Revertido el riso de 3.14.0

**revert(fx)** — La reescritura del riso (3.14.0) se retira entera: `risoInk.ts` eliminado y `applyRisoV2` restaurado sin cambios. Veredicto visual en dispositivo: *"terrible, prefiero 1000 veces el riso anterior — una textura sin sentido, mal aplicada y sin ningún tipo de sentido estético"*. En la captura, el moteado de carga de tinta se reescalaba como **bloques duros** de decenas de píxeles sobre todo el lienzo.

**El fallo de método, que importa más que el efecto**: la reescritura se validó con proxies numéricos —σ de luminancia, % de tinta conservada, luminancia de borde frente a interior— medidos sobre **una mancha sólida aislada**, nunca sobre un dibujo real y **sin mirar el resultado ni una vez**. Todas las métricas dieron bien y el efecto era inservible. Los números decían "3.7× más textura" y lo que había era ruido a bloques.

**Regla que sale de aquí**: un FX no se da por bueno con estadística. Antes de commitear hay que **renderizar un frame a archivo y mirarlo**, y sobre contenido real, no sobre un rectángulo de prueba. Ver también 3.13.3, donde un efecto correcto y medido tampoco valía nada.

Riso queda como estaba en 3.13.3. La versión sube en vez de volver a 3.13.3 porque Diorame es PWA: retroceder el número rompe el flujo de actualización del service worker.

- **Files**: `src/components/strata/canvas/risoInk.ts` (eliminado), `postProcessing.ts`, `renderPipeline.ts`, `src/constants/version.ts`, `package.json`.
...
**Decisiones**:
- **Riso conserva su pasada interna.** Delegarla obligaría a una migración en `LOAD_PROJECT` que encendería un toggle en silencio en proyectos ya guardados. Con los dos activos los fantasmas se acumulan — coherente artísticamente, y es la única opción con retrocompatibilidad exacta del `.dior`.
- **Estático, no animado.** El desregistro real de imprenta está horneado en la copia; animarlo se lee como jitter digital, no como papel. Mantiene además el export determinista. (El grunge sí se anima, pero porque simula suciedad de proyección, que sí cambia frame a frame.)
- **Dos placas en diagonales opuestas** `(2,1)` y `(-1,-2)`, con recorrido `(1 + intensity·3)·renderScale` y **offsets redondeados a entero** — un desplazamiento subpíxel remuestrea el fantasma y lo hace hervir cuando la cámara se mueve.
- **Ubicación**: fase 3, sobre el offscreen, después de riso y antes de la transferencia con aberración cromática — así los fantasmas arrastran el grano riso y viñeta/grano/grunge los ven como una sola imagen.

**Verificado** (midiendo el bbox de píxeles pintados sobre el módulo real): intensidad 0 deja el bbox intacto (no-op exacto); a 0.5 las placas caen en (+5,+3) y (−2,−5); a 1.0 en (+8,+4) y (−4,−8); con `renderScale = 2` se duplican exactamente. Dos invocaciones idénticas producen los mismos bytes — sin parpadeo entre frames.

**Nota de arquitectura**: la píldora colapsada del FXPanel **no deriva del catálogo declarativo** — son botones escritos a mano. Un FX nuevo exige tocar los dos sitios.

- **Files**: `src/components/strata/canvas/misregistration.ts` (nuevo), `src/components/strata/canvas/renderPipeline.ts`, `src/types/strataTypes.ts`, `src/components/strata/StrataContext.tsx`, `src/components/strata/fx/FXPanel.tsx`, `src/design-system/icons.ts`, `src/i18n/dictionaries/{en,es}.ts`, `src/constants/version.ts`, `package.json`.
