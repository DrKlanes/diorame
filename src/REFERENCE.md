...
| `unprojectPoint.ts` | ~135 | `unprojectCinematicPoint`, `referencePlaneZ`: screen → world for CINEMA, the algebraic inverse of `transformPoint.ts`'s cinematic branch (viewport, camera rotation, perspective scale, camera translation). MIRROR THE TWO FILES — if the forward projection changes this breaks silently, and the round-trip test is what catches it. Lens distortion and the arc/orbit pivot are NOT inverted (see the file's header for why). `referencePlaneZ` picks the mid-plane of the layers holding content, never the active layer |
